...
virt-install $MISC_OPTS $BOOT_OPTS $GRAPHICS_OPTS $RESOURCES_OPTS $CPU_OPTS $CONTROLLER_OPTS $DISK_1 $NETWORK_1$MAC_CONTROLLER1 $NETWORK_2$MAC_CONTROLLER1 $NETWORK_3$MAC_CONTROLLER1 --name=$BASE_NAME-controller1 > $BASE_NAME-controller1
virsh define controller-lab1

#Create the controller node2
virsh destroy controller-lab2
virsh undefine controller-lab2
virt-install $MISC_OPTS $BOOT_OPTS $GRAPHICS_OPTS $RESOURCES_OPTS $CPU_OPTS $CONTROLLER_OPTS $DISK_1 $NETWORK_1$MAC_CONTROLLER2 $NETWORK_2$MAC_CONTROLLER2 $NETWORK_3$MAC_CONTROLLER2 --name=$BASE_NAME-controller2 > $BASE_NAME-controller2
virsh define controller-lab2

#Create the controller node3
virsh destroy controller-lab3
virsh undefine controller-lab3
virt-install $MISC_OPTS $BOOT_OPTS $GRAPHICS_OPTS $RESOURCES_OPTS $CPU_OPTS $CONTROLLER_OPTS $DISK_1 $NETWORK_1$MAC_CONTROLLER3 $NETWORK_2$MAC_CONTROLLER3  $NETWORK_3$MAC_CONTROLLER3 --name=$BASE_NAME-controller3 > $BASE_NAME-controller3
virsh define controller-lab3

#Create the compute node1
virsh destroy compute-lab1
virsh undefine compute-lab1
virt-install $MISC_OPTS $BOOT_OPTS $GRAPHICS_OPTS $RESOURCES_OPTS $CPU_OPTS $CONTROLLER_OPTS $DISK_1 $NETWORK_1$MAC_COMPUTE1 $NETWORK_2$MAC_COMPUTE1 $NETWORK_3$MAC_COMPUTE1 --name=$BASE_NAME-compute1 > $BASE_NAME-compute1
virsh define compute-lab1

#Create the compute node2
virsh destroy compute-lab2
virsh undefine compute-lab2
virt-install $MISC_OPTS $BOOT_OPTS $GRAPHICS_OPTS $RESOURCES_OPTS $CPU_OPTS $CONTROLLER_OPTS $DISK_1 $NETWORK_1S$MAC_COMPUTE2 $NETWORK_2$MAC_COMPUTE2  $NETWORK_3$MAC_COMPUTE2 --name=$BASE_NAME-compute2 > $BASE_NAME-compute2
